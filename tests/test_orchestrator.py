...
def test_end_to_end_artifacts_delivery_and_trace(tmp_path: Path) -> None:
    ...
    feature_root.mkdir()
    (feature_root / "toy.json").write_text(
        '{"required_fields":["brand","product_name"]}',
        encoding="utf-8",
    )
    config = RuntimeConfig(
        artifact_root=tmp_path / "artifacts",
        feature_set_root=feature_root,
        browser=BrowserConfig(enabled=True, required=True, max_candidates=1),
    )
    events = []
    result = ProductURLOrchestrator(
        config,
        FakeSearch(),
        FakeAcquirer(),
        FakeBrowser(),
