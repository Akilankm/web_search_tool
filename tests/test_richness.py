...
from product_evidence_harness.constants import RICHNESS_FIELD_WEIGHTS
...
def _score(**overrides) -> float:
    base = dict(
        specs={}, attributes={}, brand="", manufacturer="", structured_eans=(), description="",
        has_price=False, image_urls=(), image_count=0, availability="", page_product_name="",
    )
...
def test_fully_populated_page_reaches_full_richness():
    score = _score(
        specs={f"k{i}": f"v{i}" for i in range(6)}, brand="Acme", manufacturer="Acme Corp",
        structured_eans=("4002051612345",), description="x" * 200, has_price=True,
        image_urls=("u1", "u2", "u3"), image_count=3, availability="in stock", page_product_name="Acme Widget",
    )
