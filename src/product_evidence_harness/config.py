...
class HarnessPolicy:
    ...
    # EAN is a strong user-provided anchor, not a sole decision-maker.
    # It must never be generated by the LLM. A retailer conflict can be a warning
    # when exact product text/form is otherwise established.
...
class HarnessBudgetConfig:
    # Supports: LLM-planned country searches, scrape/refine loop, and global fallback.
...
class HarnessConfig:
    ...
    # Output policy: default is compact CSV + markdown evidence packets + compact JSON trace.
    write_markdown_reports: bool = True
    write_trace_json: bool = True
    write_debug_csvs: bool = False

    # LLM-guided harness. When enabled, the LLM is the planner/feedback brain and
    # final exact-product adjudicator. SerpAPI and crawl4ai are tools.
    ...
    # Candidate completeness: return best available URL whenever any useful URL
    # exists, but keep exactness fields honest.
