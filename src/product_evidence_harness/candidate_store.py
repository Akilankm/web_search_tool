...
from src.product_evidence_harness.url_utils import domain_of, urls_from_text
...
class CandidateStore:
    ...
    def merge_organic(self, existing: list[URLCandidate], response: OrganicSearchResponse) -> list[URLCandidate]:
        ...
        for result in response.results:
            ...
            rec["source_types"].add(f"url_type_{classify_candidate_url(url).lower()}")
            rec["organic_count"] += 1
            rec["query_sources"].add(response.query)
            if result.position is not None:
                rec["best_position"] = min(
                    rec["best_position"] or int(result.position),
                    int(result.position),
                )
    ...
    def merge_ai(self, existing: list[URLCandidate], response: SerpAIResponse) -> list[URLCandidate]:
        ...
        for url in urls_from_text(response.markdown):
            self._merge(
                ...
                snippet=response.markdown,
                source_type="ai_markdown",
            )
        for url in self._urls_from_objects(response.text_blocks):
            self._merge(
                records,
                url,
                title="AI Mode block URL",
                snippet=response.markdown,
                source_type="ai_block",
            )
        return self._candidates(records)
    ...
    def _merge(
        ...
        *,
        ...
        rec["source_types"].add(f"url_type_{classify_candidate_url(normalized).lower()}")
    ...
    def _empty_record(self, url: str) -> dict[str, Any]:
        return {
            ...
            "source_types": {f"url_type_{classify_candidate_url(url).lower()}"},
            "query_sources": set(),
            "best_position": None,
            "organic_count": 0,
            "ai_reference_count": 0,
            "ai_declared_final": False,
            "lifecycle_status": "DISCOVERED_CANONICAL_URL",
        }

    def _record(self, candidate: URLCandidate) -> dict[str, Any]:
        canonical = canonicalize_candidate_url(candidate.url) or candidate.url
        return {
            "url": canonical,
            ...
            | {f"url_type_{classify_candidate_url(canonical).lower()}"},
    ...
    def _candidates(self, records: dict[str, dict[str, Any]]) -> list[URLCandidate]:
        ...
        def product_likelihood(candidate: URLCandidate) -> int:
            kind = classify_candidate_url(candidate.url)
            return {
                "PRODUCT_DETAIL_LIKELY": 3,
                "UNKNOWN_PRODUCT_LIKE": 2,
                "UNKNOWN": 1,
            }.get(kind, 0)
